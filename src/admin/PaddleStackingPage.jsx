...
/** The API rejects `undefined` anywhere in the payload — strip it recursively. */
...
const SKILL_OPTIONS = [
  ...
  { id: "novice_low", label: "Novice Low" },
  { id: "novice_high", label: "Novice High" },
...
export default function PaddleStackingPage() {
  ...
  const [randomSkill, setRandomSkill] = useState("beginner");
  ...
  useEffect(() => {
    ...
    async function loadCourts() {
      try {
        ...
        if (!cancelled) {
          setFacilityCourts(Array.isArray(data) ? data : []);
          setFacilityCourtsReady(true);
        }
      } catch (err) {
        console.error("Paddle stacking — courts load:", err);
        if (!cancelled) {
          setFacilityCourts([]);
          setFacilityCourtsReady(true);
        }
  ...
  useEffect(() => {
    ...
    async function loadBookings() {
      try {
        ...
        console.error("Paddle stacking — bookings load:", err);
        if (!cancelled) {
          setBookings([]);
          setBookingsReady(true);
        }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await apiGet("/api/paddle-stack/state");
        if (!mounted) return;
        const nextState = data && Object.keys(data).length ? data : null;
        setState(nextState);
        if (!nextState) {
          const initialState = { queue: [], courts: [], sessionNote: "", stackMode: "mix", updatedAt: new Date().toISOString() };
          await apiPut("/api/paddle-stack/state", initialState);
          setState(initialState);
        }
        const historyData = await apiGet("/api/paddle-match-history?limit=80");
        if (!mounted) return;
        setHistory(Array.isArray(historyData) ? historyData : []);
      } catch (err) {
        console.error("Paddle stacking — state load:", err);
      } finally {
  ...
  const persistState = useCallback(async (partial) => {
    ...
    const nextState = { ...(state || {}), ...cleaned, updatedAt: new Date().toISOString() };
    setState(nextState);
    await apiPut("/api/paddle-stack/state", nextState);
  }, [state]);
  ...
  async function finishMatch(courtId) {
    ...
    try {
      await apiPost("/api/paddle-match-history", {
        courtId: court.id,
        ...
        <div className="xl:col-span-1 space-y-4">
          <div className="ad-card p-4 transition-all duration-200">
            ...
            <p className="text-[11px] text-[var(--ad-muted)] mb-3">
              Tag category & skill for categorized players. Doubles: the top of the list decides — categorized first players get the next three matching tags in order.
            </p>
