...
export default function PaddleScorerPage() {
  ...
  useEffect(() => {
    ...
    async function loadState() {
      try {
        ...
        if (!cancelled) {
          setState(data && Object.keys(data).length ? data : null);
        }
      } catch (err) {
        console.error("Failed to load paddle stack state", err);
      } finally {
  ...
  async function updateScore(newA, newB) {
    ...
    try {
      ...
      const nextState = { ...(state || {}), courts: nextCourts, updatedAt: new Date().toISOString() };
      setState(nextState);
