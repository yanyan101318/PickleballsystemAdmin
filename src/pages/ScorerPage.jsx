...
import { subscribeToMatch, updateMatch, updateNextMatch, setChampion, getTournamentInfo, getMatch } from "../services/tournamentService";
import {
  recordSetWin,
  undoLastSet,
  setsNeeded,
...
export default function ScorerPage() {
    ...
    try {
      ...
      if (result.gameEnded) {
        ...
        if (m.winner && m.nextMatchId) {
          const nextMatch = await getMatch(tournamentId, m.nextMatchId);
          if (nextMatch) {
            const nm = { ...nextMatch };
  ...
  async function handleUndo() {
    ...
    try {
      ...
      await updateMatch(tournamentId, m);
      if (hadWinner && nextId) {
        const nextMatch = await getMatch(tournamentId, nextId);
        if (nextMatch) {
          const nm = { ...nextMatch };
